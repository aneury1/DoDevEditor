...
private:
...
#if STP_LINUX
        m_ioThread = std::thread([this]() { _IOLoop_Linux(); });
#elif STP_WINDOWS
        m_ioThread = std::thread([this]() { _IOLoop_Windows(); });
#endif
    }
    ...
    void _IOLoop_Linux()
    {
        ...
        while (m_running)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                if (events[i].data.fd != m_masterFd) continue;
    ...
    void _IOLoop_Windows()
    {
        ...
        while (m_running)
        {
            ...
            if (!ok) break; // pipe broken → child has exited
            ...
            if (c == 0x1B) // ESC
            {
                ...
                if (next == '[') // CSI: ESC [ <param bytes> <intermediate bytes> <final byte>
                {
                    ...
                    while (i < _s.size() &&
                        ...
                        ++i;
                    if (i < _s.size()) ++i; // skip final byte (0x40–0x7E)
                }
                else if (next == ']') // OSC: ESC ] ... ST or BEL
                {
                    ...
                    while (i < _s.size())
                    {
                        ...
                        if (oc == 0x07) { ++i; break; }         // BEL terminates
