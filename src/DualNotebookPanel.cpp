...
DualNotebookPanel::DualNotebookPanel(wxWindow *parent, wxWindowID id )
    : wxPanel(parent, id)
{
    BuildUI();
}

wxAuiNotebook *DualNotebookPanel::GetTopNotebook() { return m_topNotebook; }

wxAuiNotebook *DualNotebookPanel::GetBottomNotebook() { return m_bottomNotebook; }

std::optional<wxPanel *> DualNotebookPanel::getPanel(const std::string &id)
{
    auto f = m_bottom_panels.find(id);
    if (f != m_bottom_panels.end())
        return m_bottom_panels[id];
    return std::nullopt;
}

void DualNotebookPanel::BuildUI()
{
...
void DualNotebookPanel::CreateBottomTabs()
{
    struct TabDef
    {
        const char *name;
